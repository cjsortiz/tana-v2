...
public class MailService {
    ...
    @Value("${tana.mail.logo-key:tana-logo/tana-logo.png}")
    private String tanaLogoKey;
    ...
    public void sendHtmlEmail(Map<String, Object> metaData) throws MessagingException, IOException {
        ...
        List<EmailPhotoAttachment> photos =
            (List<EmailPhotoAttachment>) metaData.getOrDefault("photos", List.of());
        ...
        if (!ObjectUtils.isEmpty(photos)) {
            for (EmailPhotoAttachment photo : photos) {
                helper.addAttachment(
                    Objects.requireNonNullElse(photo.getFileName(), "image.jpg"),
                    new ByteArrayResource(photo.getBytes()),
                    photo.getContentType()
                );
            }
        }

        addTanaLogoInline(helper);


        mailSender.send(message);
        log.info("Suggestion : {} Successfully sent", metaData.get("spotName"));
    }

    private void addTanaLogoInline(MimeMessageHelper helper) {
        try {
            ResponseBytes<GetObjectResponse> logoBytes = s3Client.getObjectAsBytes(
            ...
            helper.addInline(
                "tanaLogo",
                new ByteArrayResource(logoBytes.asByteArray()),
                "image/png"
            );
        } catch (Exception e) {
            log.warn(
                "Tana email logo not found in S3: bucket={}, key={}",
                bucketName,
                tanaLogoKey,
                e
            );
